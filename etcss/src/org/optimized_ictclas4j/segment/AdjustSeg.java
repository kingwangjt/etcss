...
import org.optimized_ictclas4j.utility.NumUtil;
...
public class AdjustSeg {
                ...
                if (!isNum) {
                    ...
                    if (Utility.isDelimiter(curWord)) {
                        ...
                        if (prevsn != null && Utility.isDelimiter(prevsn.getWord())) {
                            ...
                            pos = POSTag.PUNC;
                    } else if (curWord.length() == 1 && "月日时分秒".contains(curWord) || "月份".equals(curWord)) {
     ...
     * @return
     */
    public static ArrayList<SegNode> finalAdjust(ArrayList<SegNode> optSegPath,
                                                 PosTagger personTagger,
            ...
            for (int i = 0; i < optSegPath.size(); i++) {
                ...
                else if (wr.getPos() == POSTag.NOUN_SPACE && i + 1 < optSegPath.size())// PostFix
                {
                    ...
                    if (placeTagger.getUnknownDict().isExist(next.getSrcWord(), 4)) {
                        ...
                        isBeProcess = true;
                        result.add(wr2);
                    } else if (optSegPath.get(i + 1).getLen() == 2 && "语文字杯".contains(next.getSrcWord())) {
                        SegNode wr2 = new SegNode();
                        wr2.setWord(wr.getSrcWord() + next.getSrcWord());
