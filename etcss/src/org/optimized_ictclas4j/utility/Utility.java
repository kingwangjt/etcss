...
import org.optimized_ictclas4j.bean.PersonName;
import org.optimized_ictclas4j.segment.PosTagger;
...
public class Utility {
    ...
    public static final String SEPERATOR_LINK = System.lineSeparator();
    ...
    private static final String POSTFIX_SINGLE = "坝邦堡杯城池村单岛道堤店洞渡队法峰府冈港阁宫沟国海号河湖环集江奖礁角街井郡坑口矿里岭楼路门盟庙弄牌派坡铺旗桥区渠泉人山省市水寺塔台滩坛堂厅亭屯湾文屋溪峡县线乡巷型洋窑营屿语园苑院闸寨站镇州庄族陂庵町";
    private static final String[] POSTFIX_MUTIPLE = {"半岛", "草原", "城市", "大堤", "大公国", "大桥", "地区", "帝国",
    ...
    private static final String TRANS_RUSSIAN = "·阿安奥巴比彼波布察茨大德得丁杜尔法夫伏甫盖格哈基加坚捷金卡科可克库拉莱兰勒雷里历利连列卢鲁罗洛马梅蒙米姆娜涅宁诺帕泼普奇齐乔切日萨色山申什斯索塔坦特托娃维文乌西希谢亚耶叶依伊以扎佐柴达登蒂戈果海赫华霍吉季津柯理琳玛曼穆纳尼契钦丘桑沙舍泰图瓦万雅卓兹";
    private static final String TRANS_JAPANESE = "安奥八白百邦保北倍本比滨博步部彩菜仓昌长朝池赤川船淳次村大代岛稻道德地典渡尔繁饭风福冈高工宫古谷关广桂贵好浩和合河黑横恒宏后户荒绘吉纪佳加见健江介金今进井静敬靖久酒菊俊康可克口梨理里礼栗丽利立凉良林玲铃柳隆鹿麻玛美萌弥敏木纳南男内鸟宁朋片平崎齐千前浅桥琴青清庆秋丘曲泉仁忍日荣若三森纱杉山善上伸神圣石实矢世市室水顺司松泰桃藤天田土万望尾未文武五舞西细夏宪相小孝新星行雄秀雅亚岩杨洋阳遥野也叶一伊衣逸义益樱永由有佑宇羽郁渊元垣原远月悦早造则泽增扎宅章昭沼真政枝知之植智治中忠仲竹助椎子佐阪坂堀荻菅薰浜濑鸠筱";
    ...
    public static boolean isForeign(String word) {
        if (word != null) {
            ...
            if (charCount > 2 || foreignCount >= charCount / 2)
                return true;
        }
        return false;
    }
    ...
    public static boolean PostfixSplit(byte[] sWord, byte[] sWordRet, byte[] sPostfix) {
        ...
        int nPostfixLen, nWordLen = sWord.length;
        int i = 0;

        while (sMultiPostfix[i][0] != 0
                && !strNCmp(
                GFCommon.bytesCopy(sWord, nWordLen - sMultiPostfix[i].length, sWord.length - nWordLen
                        + sMultiPostfix[i].length), 0, sMultiPostfix[i], sMultiPostfix[i].length)) {// Try
            // to
            // get
            // the
            // postfix of an
    ...
    public static boolean isYearTime(String snum) {
        if (snum != null) {
            ...
            if (isAllSingleByte(snum)
                ...
                return true;
            if (isAllNum(snum) && (len >= 6 || len == 4 && "０５６７８９".contains(first)))
                return true;
    ...
    public static PersonName chineseNameSplit(String word, PosTagger personTagger) {
        ...
        if (word != null && personTagger != null) {
            ...
            if (surNameLen != 2
                    ...
                    || getForeignCharCount(word) >= 3 && freq < personDict.getFreq("张", 1) / 40
                    && freq2 < personDict.getFreq("华", 2) / 20 || (freq < 10 && getForeignCharCount(giveName) == (len - surNameLen) / 2)))
                return null;
