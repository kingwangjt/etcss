...
import org.optimized_ictclas4j.utility.GFCommon;
...
public class Dictionary {
    ...
    public void addEntryForOnce(String word, int pos, int frequency) {
        PreWord preWord = preProcessing(word);
        WordItem wordToAdd = new WordItem(word.substring(1), word.length() - 1, pos, frequency);
        ArrayList<WordItem> wis = wts.get(preWord.getIndex()).getWords();
        int found = findNearestInOriginalTable(preWord.getIndex(), preWord.getRes(), -1);
        wis.add(found + 1, wordToAdd);
    }
            ...
            if (found >= 0) {
                ...
                if (handle == -1) {
                    for (int i = found; i < mt.getCount()
                            && strEqual(mt.getWords().get(i).getWord(), pw.getRes()); i++) {
     ...
     * 用2分法查询源词典库,看是否已存在
     *
     * @param index  大数据块对应的下标（所有同一个字开头的词条为一个大数据块）
     * @param res    去掉头一个字后剩余的部分
     * @param handle
     * @return
     */
        ...
        if (res != null && wts != null) {
            ...
            if (wt != null && wt.getCount() > 0) {
                ...
                while (start <= end) {
                    ...
                    } else if (cmpValue < 0 || cmpValue == 0 && wi.getHandle() < handle)
                        start = mid + 1;
                    else if (cmpValue > 0 || cmpValue == 0 && wi.getHandle() > handle)
                        end = mid - 1;
     ...
     * @return
     */
    public int findNearestInOriginalTable(int index, String res, int handle) {
        int result = -1;
